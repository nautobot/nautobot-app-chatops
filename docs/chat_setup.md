...
2. Use the search box to locate "Bot Channels Registration", and select "Create"
3. Configure the bot handle, subscription, resource group, and location.
...
6. Select "Create" and wait for the "Deployment succeeded" pop-up to appear in your browser, then click "Go to resource"
7. In the sidebar to the left, select "Channels" and select the "Microsoft Teams" icon. All of the default settings
   here are fine, so just click "Save".
8. In the sidebar to the left, select "Settings". Take note of the "Microsoft App ID" that is displayed, then click
   the "Manage" link above the App ID field.
   - On the new "Certificates & secrets" page, click "New client secret". (You may have to delete one of the existing
     secrets first, as there is a maximum limit). Take note of the newly generated secret, as there's no way to recover
     it later - you will have to return to this page and generate a new secret if you lose it.
9. Now to deploy the bot to your team. In the Microsoft Teams client, select "Apps" from the sidebar to the left.
   - Use the search bar to find "App Studio" and select and open it.
   - Select the "Manifest editor" tab at the top of the window.
   - Select "Import an existing app", and upload the `Nautobot_ms_teams.zip` file from this directory.
   - Update the "App ID" to the value that you took note of in step 8 above.
   - Select "Test and distribute" from the sidebar and "Download" the app package to your disk.
   - Again select "Apps" in the left sidebar, then "Upload a custom app"
   - Select your updated app package zip file.
   - After a few moments, the Nautobot app will appear in the window. Select it and click "Add".
10. Configure the `microsoft_app_id` and `microsoft_app_password` for the plugin with the App ID and client secret
    values from step 8 in your `.creds.env` file.
11. Proceed to the general process to "Grant access to the chatbot" below.
